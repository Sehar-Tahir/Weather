// pages/AdminDashboard.tsx - COMPLETE FIXED VERSION WITH UTILITY IMAGE HANDLING
      ...
      <div className="relative z-10 flex flex-col lg:flex-row min-h-screen">
        ...
        <aside className="lg:w-72 lg:min-h-screen border-r border-white/5 bg-black/40 backdrop-blur-xl p-4">
          ...
          <nav className="flex lg:flex-col gap-1">
            ...
            <button
              className={`admin-sidebar-item ${tab === 'stories' ? 'active' : ''}`}
              onClick={() => setTab('stories')}
            >
              <Newspaper size={18} /> <span>Stories</span>
            </button>
            <button
              className={`admin-sidebar-item ${tab === 'blogs' ? 'active' : ''}`}
            ...
            <button
              className={`admin-sidebar-item ${tab === 'contacts' ? 'active' : ''}`}
            ...
            {isSuperAdmin && (
              <button
                className={`admin-sidebar-item ${tab === 'admins' ? 'active' : ''}`}
          ...
          <button
            className="admin-sidebar-item text-red-400 hover:bg-red-500/10"
            onClick={logout}
          >
...
function Overview({ stories, blogs, setTab, currentAdmin }: any) {
  const stats = [
    ...
    {
      label: 'Admin Users',
      value: currentAdmin?.role === 'superadmin' ? 'Full Access' : 'Limited',
      icon: <Shield size={20} />,
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {stats.map((stat, i) => (
          ...
          >
            <div
              className={`w-12 h-12 rounded-xl bg-gradient-to-br ${stat.color} bg-opacity-20 flex items-center justify-center mb-3`}
            >
            ...
            <div className="space-y-3">
              {recentStories.map((story: any) => (
                ...
                >
                  {story.thumbnail && (
                    <img
                      src={getImageWithFallback(story.thumbnail, FALLBACK_SMALL)}
                      alt={story.title}
            ...
            <div className="space-y-3">
              {recentBlogs.map((blog: any) => (
                ...
                >
                  {blog.coverImage && (
                    <img
                      src={getImageWithFallback(blog.coverImage, FALLBACK_SMALL)}
                      alt={blog.title}
                      className="w-12 h-12 rounded-lg object-cover"
                      onError={handleImageError}
                    />
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Create and manage weather stories
          </p>
        </div>
        <button
          onClick={() => setShowForm(true)}
          className="glass-btn-primary px-5 py-2.5 flex items-center gap-2"
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {formData.thumbnail && (
                  <img
                    src={getImageWithFallback(formData.thumbnail)}
                    alt="Preview"
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {formData.coverImage && (
                  <img
                    src={getImageWithFallback(formData.coverImage)}
                    alt="Preview"
                    className="mt-2 h-24 rounded-lg object-cover"
                    onError={handleImageError}
                  />
          ...
          blogs.map((blog: any) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {blog.readTime} min read • By {blog.author} •{' '}
                  {new Date(blog.createdAt).toLocaleDateString()}
                </p>
              </div>
              <div className="flex gap-2">
...
function AdminsManager() {
  const { admins, createAdmin, updateAdmin, deleteAdmin, refreshStories } =
    useApp();
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.email || !formData.name || (!editing && !formData.password))
      return;
            ...
            >
              ...
              >
                <Shield
                  size={20}
                  className={
                    admin.role === 'superadmin' ? 'text-purple-400' : 'text-sky-400'
                  }
                />
              </div>
              <div className="flex-1">
...
function ContactsManager() {
  const { contacts = [], refreshContacts, updateContactStatus, deleteContact } =
    useApp();
      ...
      {filteredContacts.length === 0 ? (
        ...
        <div className="space-y-6">
          {filteredContacts.map((contact: any) => (
            <div
              key={contact._id}
              className="glass-card p-6 rounded-2xl hover-lift border border-white/10"
            >
              <div className="flex justify-between items-start mb-5">
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-xs px-4 py-1.5 rounded-full ${getStatusColor(
                      contact.status
                    )}`}
                  >
...
// ── EXPORT ────────────────────────────────────────────────────
// ✅ SINGLE DEFAULT EXPORT - THIS IS CRITICAL
